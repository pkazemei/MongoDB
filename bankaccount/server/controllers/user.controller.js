...
class UserController {
        ...
        User.find({userName: req.body.userName})
            .then(foundUser=>{
                ...
                if(foundUser.length===0){
                    ...
                    res.json({errors: {usernameTaken:"username is taken"}})
                }
            })
            .catch(err=>res.json({err}))
    ...
    login = async (req, res) => {
        //find a user in mongodb by the username from the form
        console.log('heeey',req.body);
        ...
        if(user === null) {
            ...
            return res.json({msg: "username not found. Who dis?"})
        }
    ...
    deleteUser = (req,res)=>{
        const decodedJWT = jwt.decode(req.cookies.usertoken, { complete: true });
        User.deleteOne({_id : decodedJWT.payload.id}, req.body)
            .then(deletedUser=>{
    ...
    editUser = (req, res) => {
        const decodedJWT = jwt.decode(req.cookies.usertoken, { complete: true });
        console.log('hereeeeee', decodedJWT)
